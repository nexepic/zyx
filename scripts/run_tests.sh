...
# Use Conan native file if available (generated by conan install), otherwise use system defaults
NATIVE_FILE_ARG=""
if [ -f "$BUILD_DIR/conan_meson_native.ini" ]; then
    NATIVE_FILE_ARG="--native-file $BUILD_DIR/conan_meson_native.ini"
fi

if [ ! -f "$BUILD_DIR/build.ninja" ]; then
    meson setup "$BUILD_DIR" $NATIVE_FILE_ARG \
