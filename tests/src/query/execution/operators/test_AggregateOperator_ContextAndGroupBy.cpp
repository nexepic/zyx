/**
 * @file test_AggregateOperator_ContextAndGroupBy.cpp
...
class AggregateContextAndGroupByMockOperator : public PhysicalOperator {
public:
	explicit AggregateContextAndGroupByMockOperator(std::vector<RecordBatch> batches) : batches_(std::move(batches)) {}
...
TEST(AggregateOperatorContextAndGroupByTest, UsesQueryContextParametersForCountAndSumExpressions) {
	Record r1;
	Record r2;
	auto child = std::make_unique<AggregateContextAndGroupByMockOperator>(std::vector<RecordBatch>{{r1, r2}});
...
TEST(AggregateOperatorContextAndGroupByTest, GroupByNullExpressionUsesNullKeyPath) {
	Record r1;
...
TEST(AggregateOperatorContextAndGroupByTest, ToStringCoversCountCase) {
	auto child = std::make_unique<AggregateContextAndGroupByMockOperator>(std::vector<RecordBatch>{});
	std::vector<AggregateItem> aggs;
	aggs.emplace_back(AggregateFunctionType::AGG_COUNT, nullptr, "cnt");
