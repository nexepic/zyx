/**
 * @file test_VectorSearchOperator_InvalidNode.cpp
...
class VectorSearchOperatorInvalidNodeTest : public ::testing::Test {
protected:
	fs::path dbPath;
	...
	void SetUp() override {
		...
		dbPath = fs::temp_directory_path() / ("test_vector_search_branches_" + boost::uuids::to_string(uuid) + ".zyx");
		if (fs::exists(dbPath)) {
			fs::remove(dbPath);
		}
...
TEST_F(VectorSearchOperatorInvalidNodeTest, NextSkipsNodeResolvedAsInvalidFromSearchResults) {
