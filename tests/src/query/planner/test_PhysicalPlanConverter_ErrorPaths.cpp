/**
 * @file test_PhysicalPlanConverter_ErrorPaths.cpp
...
class PhysicalPlanConverterErrorPathsTest : public ::testing::Test {
protected:
...
TEST_F(PhysicalPlanConverterErrorPathsTest, ConvertRejectsNullLogicalOperator) {
	EXPECT_THROW((void)converter->convert(nullptr), std::invalid_argument);
}

TEST_F(PhysicalPlanConverterErrorPathsTest, ConvertRejectsUnsupportedLogicalType) {
	InvalidLogicalOperator invalid;
	EXPECT_THROW((void)converter->convert(&invalid), std::runtime_error);
}

TEST_F(PhysicalPlanConverterErrorPathsTest, ConvertTransactionControlCoversAllCommands) {
...
TEST_F(PhysicalPlanConverterErrorPathsTest, ConvertTransactionControlRejectsUnknownCommand) {
...
TEST_F(PhysicalPlanConverterErrorPathsTest, ConvertAggregateCoversCollectAndAliasFallback) {
...
TEST_F(PhysicalPlanConverterErrorPathsTest, ConvertAggregateRejectsUnknownFunctionName) {
	auto child = std::make_unique<LogicalSingleRow>();
	std::vector<std::shared_ptr<Expression>> groupByExprs;
	std::vector<LogicalAggItem> aggs;
...
TEST_F(PhysicalPlanConverterErrorPathsTest, ConvertSetCoversMapMergeAndDefaultAndNullChild) {
...
TEST_F(PhysicalPlanConverterErrorPathsTest, ConvertDeleteHandlesNullChildPath) {
...
TEST_F(PhysicalPlanConverterErrorPathsTest, ConvertRemoveCoversDefaultAndNullChild) {
