...
TEST(RelationshipCountScanPlannerTest, AcceptsOneHopCountTraversal) {
	LogicalAggregate aggregate(makeOneHop(), {}, makeAggs());

	auto plan = tryBuildRelationshipCountScanPlan(aggregate);
...
TEST(RelationshipCountScanPlannerTest, AcceptsTwoHopCountTraversal) {
...
TEST(RelationshipCountScanPlannerTest, AcceptsCountStarAndCountEdgeVariable) {
	LogicalAggregate countStar(makeOneHop(), {}, makeAggs(nullptr));
	EXPECT_TRUE(tryBuildRelationshipCountScanPlan(countStar).has_value());

	LogicalAggregate countEdge(makeOneHop(), {}, makeAggs(std::make_shared<expressions::VariableReferenceExpression>("r")));
	EXPECT_TRUE(tryBuildRelationshipCountScanPlan(countEdge).has_value());
}

TEST(RelationshipCountScanPlannerTest, RejectsAnchoredSeedWithoutIndexCandidateSource) {
	...
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(aggregate).has_value());
}

TEST(RelationshipCountScanPlannerTest, PreservesUnanchoredRelationshipTypeScanPlan) {
...
TEST(RelationshipCountScanPlannerTest, UsesDirectRelationshipCountForUnanchoredEdgeCountWithoutProperties) {
...
TEST(RelationshipCountScanPlannerTest, UsesDirectRelationshipCountForUnanchoredEdgePropertyCount) {
	auto seed = std::make_unique<LogicalNodeScan>("u", std::vector<std::string>{});
	auto hop = std::make_unique<LogicalTraversal>(std::move(seed), "u", "r", "v", "FOLLOWS", "out",
...
TEST(RelationshipCountScanPlannerTest, UsesDirectRelationshipCountForUnanchoredEdgeFilterCount) {
	auto seed = std::make_unique<LogicalNodeScan>("u", std::vector<std::string>{});
	auto hop = std::make_unique<LogicalTraversal>(std::move(seed), "u", "r", "v", "FOLLOWS", "out");
...
TEST(RelationshipCountScanPlannerTest, AllowsDirectRelationshipCountWithEmptyFilterPredicate) {
	auto seed = std::make_unique<LogicalNodeScan>("u", std::vector<std::string>{});
	auto hop = std::make_unique<LogicalTraversal>(std::move(seed), "u", "r", "v", "FOLLOWS", "out");
...
TEST(RelationshipCountScanPlannerTest, RejectsDirectCountArgumentsThatAreNotEdgeVariables) {
	...
	auto targetVariablePlan = tryBuildRelationshipCountScanPlan(targetVariableAggregate);
	...
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(edgePropertyAggregate).has_value());
	...
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(literalAggregate).has_value());
}

TEST(RelationshipCountScanPlannerTest, ExtractsDirectEdgeFilterLiteralVariants) {
	auto seed = std::make_unique<LogicalNodeScan>("u", std::vector<std::string>{});
	auto hop = std::make_unique<LogicalTraversal>(std::move(seed), "u", "r", "v", "FOLLOWS", "out");
...
TEST(RelationshipCountScanPlannerTest, ExtractsDirectEdgeFilterNullAndDoubleLiterals) {
	auto seed = std::make_unique<LogicalNodeScan>("u", std::vector<std::string>{});
	auto hop = std::make_unique<LogicalTraversal>(std::move(seed), "u", "r", "v", "FOLLOWS", "out");
	auto left = makeEdgeEquality(
...
TEST(RelationshipCountScanPlannerTest, RejectsUnsupportedDirectEdgeFilters) {
	...
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(makeAggregate(nonLiteral)).has_value());

	auto plainExpression = std::make_shared<expressions::VariableReferenceExpression>("r", "weight");
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(makeAggregate(plainExpression)).has_value());
}

TEST(RelationshipCountScanPlannerTest, RejectsConflictingDirectEdgePropertyFilters) {
	auto seed = std::make_unique<LogicalNodeScan>("u", std::vector<std::string>{});
	auto hop = std::make_unique<LogicalTraversal>(std::move(seed), "u", "r", "v", "FOLLOWS", "out",
	                                             std::vector<std::string>{},
	                                             std::vector<std::pair<std::string, PropertyValue>>{},
	...
	auto predicate = std::make_unique<expressions::BinaryOpExpression>(
			std::make_unique<expressions::VariableReferenceExpression>("r", "weight"),
			expressions::BinaryOperatorType::BOP_EQUAL,
			std::make_unique<expressions::LiteralExpression>(int64_t{2}));
	std::shared_ptr<expressions::Expression> sharedPredicate(std::move(predicate));
	auto filter = std::make_unique<LogicalFilter>(std::move(hop), std::move(sharedPredicate));
	LogicalAggregate aggregate(std::move(filter), {}, makeAggs(std::make_shared<expressions::VariableReferenceExpression>("r")));
...
TEST(RelationshipCountScanPlannerTest, ExtractsDirectEdgeComparisonPredicates) {
	auto seed = std::make_unique<LogicalNodeScan>("u", std::vector<std::string>{});
	auto hop = std::make_unique<LogicalTraversal>(std::move(seed), "u", "r", "v", "FOLLOWS", "out");
...
TEST(RelationshipCountScanPlannerTest, AddsSeedRangePredicatesNotCoveredByCandidateSource) {
...
TEST(RelationshipCountScanPlannerTest, AddsOpenSeedRangeBoundsWhenNotCoveredByCandidateSource) {
	...
	auto minOnlyPlan = tryBuildRelationshipCountScanPlan(minOnlyAggregate);
	...
	auto maxOnlyPlan = tryBuildRelationshipCountScanPlan(maxOnlyAggregate);
	...
	auto inclusiveMinPlan = tryBuildRelationshipCountScanPlan(inclusiveMinAggregate);
	...
	auto inclusiveMaxPlan = tryBuildRelationshipCountScanPlan(inclusiveMaxAggregate);
...
TEST(RelationshipCountScanPlannerTest, AddsExclusiveRangePairAndKeepsRequiredPropertiesUnique) {
	auto seed = makeSeedScan();
...
TEST(RelationshipCountScanPlannerTest, RejectsSeedRangeWithoutUsableBounds) {
	auto seed = makeSeedScan();
...
TEST(RelationshipCountScanPlannerTest, UsesDefaultAliasWhenAggregateAliasIsEmpty) {
	std::vector<LogicalAggItem> aggs;
...
TEST(RelationshipCountScanPlannerTest, UsesPreferredRangeAndCompositeCandidateSources) {
	...
	auto rangePlan = tryBuildRelationshipCountScanPlan(rangeAggregate);
	...
	auto compositePlan = tryBuildRelationshipCountScanPlan(compositeAggregate);
...
TEST(RelationshipCountScanPlannerTest, AddsPredicateForPreferredOpenRangeCandidateSource) {
	...
	auto minPlan = tryBuildRelationshipCountScanPlan(minAggregate);
	...
	auto maxPlan = tryBuildRelationshipCountScanPlan(maxAggregate);
...
TEST(RelationshipCountScanPlannerTest, RejectsInvalidCompositeSeedCandidate) {
...
TEST(RelationshipCountScanPlannerTest, AllowsSeedAndSourceCountVariablesButRejectsInvalidCountArguments) {
	LogicalAggregate seedCount(makeOneHop(), {}, makeAggs(std::make_shared<expressions::VariableReferenceExpression>("u")));
	EXPECT_TRUE(tryBuildRelationshipCountScanPlan(seedCount).has_value());
	...
	EXPECT_TRUE(tryBuildRelationshipCountScanPlan(sourceCount).has_value());

	LogicalAggregate propertyCount(makeOneHop(), {}, makeAggs(std::make_shared<expressions::VariableReferenceExpression>("v", "id")));
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(propertyCount).has_value());

	LogicalAggregate unknownCount(makeOneHop(), {}, makeAggs(std::make_shared<expressions::VariableReferenceExpression>("missing")));
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(unknownCount).has_value());

	LogicalAggregate literalCount(makeOneHop(), {}, makeAggs(std::make_shared<expressions::LiteralExpression>(int64_t{1})));
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(literalCount).has_value());
}

TEST(RelationshipCountScanPlannerTest, RejectsDirectCountShapeWhenOnlyTraversalPathCanHandleIt) {
	...
	auto inwardPlan = tryBuildRelationshipCountScanPlan(inwardAggregate);
	...
	auto labeledTargetPlan = tryBuildRelationshipCountScanPlan(labeledTargetAggregate);
	...
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(targetPropertyAggregate).has_value());
}

TEST(RelationshipCountScanPlannerTest, RejectsSelectiveSeedWhenPreferredSourceIsNotIndexBacked) {
	...
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(unanchoredPropertyAggregate).has_value());
	...
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(labelAggregate).has_value());

	auto compositeSeed = std::make_unique<LogicalNodeScan>("u", std::vector<std::string>{"User"});
	compositeSeed->setCompositeEquality({{"country", "age"}, {PropertyValue("CN"), PropertyValue(int64_t{30})}});
	LogicalAggregate compositeAggregate(makeOneHopFrom(std::move(compositeSeed)), {}, makeAggs());
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(compositeAggregate).has_value());
}

TEST(RelationshipCountScanPlannerTest, HandlesPreferredCandidateSourcesWithoutPredicates) {
	...
	auto propertyPlan = tryBuildRelationshipCountScanPlan(propertyAggregate);
...
TEST(RelationshipCountScanPlannerTest, RejectsMalformedTraversalShapes) {
	LogicalAggregate missingChild(nullptr, {}, makeAggs());
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(missingChild).has_value());
	...
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(missingFilterChild).has_value());
	...
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(missingTraversalChild).has_value());
	...
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(threeHopAggregate).has_value());
}

TEST(RelationshipCountScanPlannerTest, ChoosesIndexedRangeCompositeAndLabelCandidateSources) {
	...
	rangeSeed->setRangePredicates({RangePredicate{"age", PropertyValue(int64_t{18}), PropertyValue(int64_t{65}), true, false}});
	LogicalAggregate rangeAggregate(makeOneHopFrom(std::move(rangeSeed)), {}, makeAggs());
	auto rangePlan = tryBuildRelationshipCountScanPlan(rangeAggregate, indexManager);
	ASSERT_TRUE(rangePlan.has_value());
	EXPECT_EQ(rangePlan->seedConfig.type, execution::ScanType::RANGE_SCAN);
	EXPECT_TRUE(rangePlan->seedPredicates.empty());
	...
	compositeSeed->setCompositeEquality({{"country", "age"}, {PropertyValue("CN"), PropertyValue(int64_t{30})}});
	LogicalAggregate compositeAggregate(makeOneHopFrom(std::move(compositeSeed)), {}, makeAggs());
	auto compositePlan = tryBuildRelationshipCountScanPlan(compositeAggregate, indexManager);
	ASSERT_TRUE(compositePlan.has_value());
	EXPECT_EQ(compositePlan->seedConfig.type, execution::ScanType::COMPOSITE_SCAN);

	LogicalAggregate labelAggregate(makeOneHopFrom(std::make_unique<LogicalNodeScan>("u", std::vector<std::string>{"User"})), {}, makeAggs());
	auto labelPlan = tryBuildRelationshipCountScanPlan(labelAggregate, indexManager);
...
TEST(RelationshipCountScanPlannerTest, RejectsSelectiveSeedWhenIndexManagerHasNoMatchingIndex) {
	const auto dbPath = fs::temp_directory_path() /
	...
	LogicalAggregate rangeAggregate(makeOneHopFrom(std::move(rangeSeed)), {}, makeAggs());
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(rangeAggregate, indexManager).has_value());
...
TEST(RelationshipCountScanPlannerTest, DirectRelationshipCountCarriesPlannedRelationshipCandidateSource) {
	const auto dbPath = fs::temp_directory_path() /
	                    ("test_relationship_count_planner_relationship_access_" + boost::uuids::to_string(boost::uuids::random_generator()()) + ".zyx");
	Database db(dbPath.string());
	db.open();
	auto indexManager = db.getQueryEngine()->getIndexManager();
	ASSERT_TRUE(indexManager->createIndex("idx_edge_type_relationship_access", "edge", "", ""));
	ASSERT_TRUE(indexManager->createIndex("idx_edge_weight_relationship_access", "edge", "", "weight"));
	...
	ASSERT_TRUE(plan.has_value());
	ASSERT_TRUE(plan->relationshipAccessPath.has_value());
	EXPECT_EQ(plan->relationshipAccessPath->kind, "type_property_intersection");
	EXPECT_EQ(plan->directCount.candidateSource.type,
	          execution::DirectRelationshipCandidateSourceType::DRCS_TYPE_PROPERTY_INTERSECTION);
	EXPECT_EQ(plan->directCount.candidateSource.propertyKeys, (std::vector<std::string>{"weight"}));
...
TEST(RelationshipCountScanPlannerTest, RejectsDistinctGroupedAndPropertyFilters) {
	LogicalAggregate distinct(makeOneHop(), {}, makeAggs(std::make_shared<expressions::VariableReferenceExpression>("v"), true));
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(distinct).has_value());
	...
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(grouped).has_value());
	...
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(edgeProps).has_value());
	...
	EXPECT_FALSE(tryBuildRelationshipCountScanPlan(targetProps).has_value());
}

TEST(RelationshipCountScanPlannerTest, PhysicalPlanConverterUsesScanPlanForRecognizedTraversalAggregate) {
	LogicalAggregate aggregate(makeOneHop(), {}, makeAggs());
	...
	EXPECT_EQ(physical->toString(), "RelationshipCountScan(count)");
}

TEST(RelationshipCountScanPlannerTest, PhysicalPlanConverterPropagatesAccessPathExplainAttributes) {
	const auto dbPath = fs::temp_directory_path() /
	                    ("test_relationship_count_planner_explain_attributes_" + boost::uuids::to_string(boost::uuids::random_generator()()) + ".zyx");
	Database db(dbPath.string());
	db.open();
	auto dataManager = db.getStorage()->getDataManager();
	auto indexManager = db.getQueryEngine()->getIndexManager();
	ASSERT_TRUE(indexManager->createIndex("idx_edge_type_explain_access", "edge", "", ""));
	ASSERT_TRUE(indexManager->createIndex("idx_edge_weight_explain_access", "edge", "", "weight"));
	...
	auto hop = std::make_unique<LogicalTraversal>(std::move(seed), "u", "r", "v", "FOLLOWS", "out",
	                                             ...
	                                             std::unordered_map<std::string, PropertyValue>{{"weight", PropertyValue(int64_t{1})}});
	LogicalAggregate aggregate(std::move(hop), {}, makeAggs(std::make_shared<expressions::VariableReferenceExpression>("r")));
	PhysicalPlanConverter converter(dataManager, indexManager);

	auto physical = converter.convert(&aggregate);

	ASSERT_NE(physical, nullptr);
	const auto attributes = physical->explainAttributes();
	auto hasAttribute = [&](const std::string &key, const std::string &value) {
		return std::any_of(attributes.begin(), attributes.end(), [&](const auto &attribute) {
			return attribute.first == key && attribute.second == value;
		});
	};
	EXPECT_TRUE(hasAttribute("seed_access_path.kind", "full_scan"));
	EXPECT_TRUE(hasAttribute("relationship_access_path.kind", "type_property_intersection"));
	EXPECT_TRUE(hasAttribute("relationship_access_path.direct_candidate_lookup", "true"));
