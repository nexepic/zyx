...
TEST(NodeTopKFastPathPlannerTest, AcceptsSimpleProjectedNodePropertyTopK) {
	auto project = makeTopKProject();

	auto plan = tryBuildNodeTopKFastPathPlan(*project);
...
TEST(NodeTopKFastPathPlannerTest, RejectsUnsupportedShapes) {
	...
	EXPECT_FALSE(tryBuildNodeTopKFastPathPlan(distinct).has_value());
	...
	EXPECT_FALSE(tryBuildNodeTopKFastPathPlan(*badProject).has_value());
	...
	EXPECT_FALSE(tryBuildNodeTopKFastPathPlan(variableSort).has_value());
}

TEST(NodeTopKFastPathPlannerTest, AddsResidualPredicatesAndUsesIndexes) {
	...
	auto indexedPlan = tryBuildNodeTopKFastPathPlan(*indexedProject, indexManager);
	...
	auto residualPlan = tryBuildNodeTopKFastPathPlan(*residualProject);
...
TEST(NodeTopKFastPathPlannerTest, PhysicalPlanConverterUsesTopKFastPath) {
	auto uuid = boost::uuids::random_generator()();
	...
	EXPECT_NE(dynamic_cast<execution::operators::NodeTopKFastPathOperator *>(physical.get()), nullptr);
...
TEST(NodeTopKFastPathPlannerTest, RejectsAdditionalUnsupportedShapes) {
	...
	EXPECT_FALSE(tryBuildNodeTopKFastPathPlan(*emptyItemsProject).has_value());
	...
	EXPECT_FALSE(tryBuildNodeTopKFastPathPlan(nonSortChild).has_value());
	...
	EXPECT_FALSE(tryBuildNodeTopKFastPathPlan(multiSortProject).has_value());
	...
	EXPECT_FALSE(tryBuildNodeTopKFastPathPlan(singleRowProject).has_value());
	...
	EXPECT_FALSE(tryBuildNodeTopKFastPathPlan(literalSortProject).has_value());
...
TEST(NodeTopKFastPathPlannerTest, BuildsRangeAndCompositeIndexPlans) {
	auto uuid = boost::uuids::random_generator()();
	...
	auto rangePlan = tryBuildNodeTopKFastPathPlan(*rangeProject, indexManager);
	...
	auto compositePlan = tryBuildNodeTopKFastPathPlan(*compositeProject, indexManager);
...
TEST(NodeTopKFastPathPlannerTest, AddsResidualRangeAndCompositePredicates) {
	...
	auto closedRangePlan = tryBuildNodeTopKFastPathPlan(*closedRangeProject);
	...
	auto lowerRangePlan = tryBuildNodeTopKFastPathPlan(*lowerRangeProject);
	...
	auto upperRangePlan = tryBuildNodeTopKFastPathPlan(*upperRangeProject);
	...
	EXPECT_FALSE(tryBuildNodeTopKFastPathPlan(*emptyRangeProject).has_value());
	...
	compositeScan->setCompositeEquality(std::move(composite));
	auto compositeProject = makeTopKProject(std::move(compositeScan));
	auto compositePlan = tryBuildNodeTopKFastPathPlan(*compositeProject);
	ASSERT_TRUE(compositePlan.has_value());
	...
	EXPECT_FALSE(tryBuildNodeTopKFastPathPlan(*malformedProject).has_value());
}

TEST(NodeTopKFastPathPlannerTest, FallsBackForInvalidPreferredScanConfig) {
	...
	auto propertyPlan = tryBuildNodeTopKFastPathPlan(*propertyProject);
	...
	auto unlabeledRangePlan = tryBuildNodeTopKFastPathPlan(*unlabeledRangeProject);
...
TEST(NodeTopKFastPathPlannerTest, HandlesPreferredScanConfigsWithoutIndexManager) {
	...
	auto rangeProject = makeTopKProject(std::move(rangeScan));
	auto rangePlan = tryBuildNodeTopKFastPathPlan(*rangeProject);
	ASSERT_TRUE(rangePlan.has_value());
	EXPECT_EQ(rangePlan->config.type, execution::ScanType::RANGE_SCAN);
	EXPECT_TRUE(rangePlan->predicates.empty());
	...
	auto labelPlan = tryBuildNodeTopKFastPathPlan(*labelProject);
	ASSERT_TRUE(labelPlan.has_value());
	EXPECT_EQ(labelPlan->config.type, execution::ScanType::LABEL_SCAN);
}

TEST(NodeTopKFastPathPlannerTest, HandlesOpenRangeFallbackAndUpperInclusivePredicate) {
	...
	auto minOpenPlan = tryBuildNodeTopKFastPathPlan(*minOpenProject);
	...
	auto maxOpenPlan = tryBuildNodeTopKFastPathPlan(*maxOpenProject);
...
TEST(NodeTopKFastPathPlannerTest, HandlesIndexSelectionFallbackBranches) {
	auto uuid = boost::uuids::random_generator()();
	...
	auto compositeNoIndexPlan = tryBuildNodeTopKFastPathPlan(*compositeNoIndexProject, indexManager);
	...
	auto unindexedPropertyPlan = tryBuildNodeTopKFastPathPlan(*unindexedPropertyProject, indexManager);
	...
	auto openRangeWithIndexPlan = tryBuildNodeTopKFastPathPlan(*openRangeWithIndexProject, indexManager);
	...
	auto unlabeledPlan = tryBuildNodeTopKFastPathPlan(*unlabeledProject, indexManager);
...
TEST(NodeTopKFastPathPlannerTest, RejectsNullLogicalChildren) {
	std::vector<LogicalProjectItem> projectItems;
	projectItems.emplace_back(property("u", "id"), "id");
	LogicalProject nullProjectChild(nullptr, projectItems);
	EXPECT_FALSE(tryBuildNodeTopKFastPathPlan(nullProjectChild).has_value());

	auto nullLimit = std::make_unique<LogicalLimit>(nullptr, 1);
	LogicalProject nullLimitChild(std::move(nullLimit), projectItems);
	EXPECT_FALSE(tryBuildNodeTopKFastPathPlan(nullLimitChild).has_value());
	...
	EXPECT_FALSE(tryBuildNodeTopKFastPathPlan(nullSortChild).has_value());
}
