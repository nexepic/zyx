...
protected:
    ...
    std::filesystem::path writeSmallDataset() {
        ...
        writeFile(dataset / "users.csv",
                  "id,age,country,score\n"
                  "user-000001,31,CN,901.5\n"
                  "user-000006,36,US,850.0\n"
                  "user-000007,42,CN,990.0\n");
        writeFile(dataset / "posts.csv",
                  "id,created_at,score\n"
                  "post-000001,1,10.5\n");
        writeFile(dataset / "tags.csv",
                  "id,rank\n"
                  "tag-000001,1\n");
        writeFile(dataset / "follows.csv",
                  "src,dst,weight\n"
                  "user-000001,user-000006,1\n"
                  "user-000006,user-000007,2\n");
        writeFile(dataset / "authored.csv",
                  "src,dst,weight\n"
                  "user-000001,post-000001,1\n");
        writeFile(dataset / "has_tag.csv",
                  "src,dst,weight\n"
...
TEST_F(CompareBenchmarkTest, ParseArgsAcceptsCompleteScanAndIndexedOptions) {
    std::vector<std::string> args = {
            "zyx-compare-bench", "--dataset", "data", "--db-path", "db", "--scale", "small",
            "--profile", "indexed", "--emit-profile", "--warmup", "2", "--iterations", "3"};
...
TEST_F(CompareBenchmarkTest, ParseArgsRejectsInvalidAndMissingValues) {
    ...
    EXPECT_THROW(parse({"zyx-compare-bench", "--dataset", "data", "--db-path", "db", "--scale", "small",
                        "--profile", "bad"}), std::invalid_argument);
    EXPECT_THROW(parse({"zyx-compare-bench", "--dataset", "data", "--db-path", "db", "--scale", "small",
                        "--warmup", "-1"}), std::invalid_argument);
    EXPECT_THROW(parse({"zyx-compare-bench", "--dataset", "data", "--db-path", "db", "--scale", "small",
                        "--iterations", "0"}), std::invalid_argument);
    EXPECT_THROW(parse({"zyx-compare-bench", "--dataset", "data", "--db-path", "db", "--scale", "small",
                        "--unknown"}), std::invalid_argument);
