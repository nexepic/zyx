...
protected:
    void SetUp() override {
        ...
        stateManager = std::make_unique<graph::storage::state::SystemStateManager>(
...
TEST_F(SystemStateManagerTest, SetAndGetMap) {
    ...
    std::unordered_map<std::string, int64_t> inputMap = {
        {"age", 100},
        {"salary", 200},
        {"id", 300}
    };
...
TEST_F(SystemStateManagerTest, Persistence) {
    ...
    newDb->open();
    auto newManager = std::make_unique<graph::storage::state::SystemStateManager>(
        newDb->getStorage()->getDataManager()
    );
