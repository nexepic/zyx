...
protected:
    ...
    void SetUp() override {
...

        {
            ...
            for(int i=0; i<10; ++i) out.write(zeros.data(), TOTAL_SEGMENT_SIZE);
        }

        fileStream = std::make_shared<std::fstream>(
            testFilePath, std::ios::binary | std::ios::in | std::ios::out);
...
TEST_F(SegmentIndexManagerTest, InvalidTypeThrows) {
    ...
    EXPECT_THROW({
        indexManager->findSegmentForId(999, 100);
...
TEST_F(SegmentIndexManagerTest, TypesAreIsolated) {
    ...
    // Node ID 100
    indexManager->updateSegmentIndex(
        createHeader(offNode, static_cast<uint32_t>(graph::EntityType::Node), 100, 10), -1);

    // Edge ID 100
    indexManager->updateSegmentIndex(
        createHeader(offEdge, static_cast<uint32_t>(graph::EntityType::Edge), 100, 10), -1);
