...
protected:
	...
	[[nodiscard]] bool hasIndexMetadata(const std::string &targetName, const std::string &targetProp) const {
		...
		for (const auto &[name, entityType, indexType, label, prop]: list) {
...
TEST_F(IndexManagerTest, CreateAndDropNamedIndex) {
	...
	for (const auto &row: list) {
		if (std::get<0>(row) == "idx_user_age") {
			...
			EXPECT_EQ(std::get<3>(row), "User");
			EXPECT_EQ(std::get<4>(row), "age");
			found = true;
		}
	}
	EXPECT_TRUE(found);
...
TEST_F(IndexManagerTest, DropIndexByDefinition) {
	...
	std::string autoName;
	for (const auto &row: list) {
		if (std::get<1>(row) == "node" && std::get<3>(row) == "Product" && std::get<4>(row) == "sku") {
...
TEST_F(IndexManagerTest, CreateVectorIndex_EmptyName) {
	...
	bool found = false;
	for (const auto &row: list) {
		if (std::get<1>(row) == "node" && std::get<3>(row) == "Node" && std::get<4>(row) == "embedding") {
			found = true;
...
TEST_F(IndexManagerTest, CreateIndex_EmptyName) {
	...
	bool found = false;
	for (const auto &row : list) {
		if (std::get<3>(row) == "AutoLabel" && std::get<4>(row) == "auto_prop") {
			found = true;
