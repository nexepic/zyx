/**
 * @file test_ConstraintManager_Branches.cpp
...
class ConstraintManagerBranchesTest : public ::testing::Test {
protected:
...
TEST_F(ConstraintManagerBranchesTest, UniqueConstraintRequiresExactlyOneProperty) {
	EXPECT_THROW(
		manager->createConstraint("uq_invalid", "node", "unique", "Person", {}, ""),
		std::runtime_error);
}

TEST_F(ConstraintManagerBranchesTest, NotNullConstraintRequiresExactlyOneProperty) {
	EXPECT_THROW(
		manager->createConstraint("nn_invalid", "node", "not_null", "Person", {"a", "b"}, ""),
		std::runtime_error);
}

TEST_F(ConstraintManagerBranchesTest, PropertyTypeConstraintRequiresExactlyOneProperty) {
	EXPECT_THROW(
		manager->createConstraint("type_invalid", "node", "property_type", "Person", {}, "INTEGER"),
		std::runtime_error);
}

TEST_F(ConstraintManagerBranchesTest, NodeKeyConstraintRequiresAtLeastOneProperty) {
	EXPECT_THROW(
		manager->createConstraint("nk_invalid", "node", "node_key", "Person", {}, ""),
		std::runtime_error);
}

TEST_F(ConstraintManagerBranchesTest, UnknownConstraintTypeThrows) {
	EXPECT_THROW(
