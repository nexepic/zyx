...
class RelationshipExpandFastPathIntegrationTest : public ::testing::Test {
protected:
	void SetUp() override {
		const auto uuid = boost::uuids::random_generator()();
		testDbPath = fs::temp_directory_path() / ("test_relationship_expand_fast_path_" + boost::uuids::to_string(uuid) + ".zyx");
...
TEST_F(RelationshipExpandFastPathIntegrationTest, CountsOneHopExpandWithFastPathProfilePhase) {
...
TEST_F(RelationshipExpandFastPathIntegrationTest, ReturningTraversalRowsUsesLegacyPath) {
	debug::PerfTrace::setEnabled(true);
	debug::PerfTrace::reset();
...
TEST_F(RelationshipExpandFastPathIntegrationTest, EdgePropertyFilterUsesLegacyPath) {
	debug::PerfTrace::setEnabled(true);
	debug::PerfTrace::reset();
