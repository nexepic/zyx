...
- **[GDS 图投影](graph-projections)** — 带 catalog 生命周期和惰性 CSR cache 的命名、筛选、带权内存图
- **[社区检测](community-detection)** — Leiden 算法，通过层级提升和精化发现社区结构
- **[PageRank](https://en.wikipedia.org/wiki/PageRank)** — 迭代式中心性算法（通过 GDS）
- **[最短路径](https://en.wikipedia.org/wiki/Shortest_path_problem)** — Dijkstra 加权最短路径算法（通过 GDS）
