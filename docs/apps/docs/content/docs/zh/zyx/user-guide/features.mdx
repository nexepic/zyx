...
| 算法 | 过程 | 说明 |
|------|------|------|
| 图投影 Catalog | `gds.graph.*` | 命名筛选图、生命周期、schema 与内存指标 |
| PageRank | `gds.pageRank.stream` | 迭代式重要性评分 |
| Leiden 社区检测 | `gds.leiden.stream` | 带精化和层级提升的 Leiden 算法 |
...
投影配置与 catalog 生命周期见 [GDS 图投影](/zh/docs/zyx/algorithms/graph-projections)。
...
ZYX 提供多套 API，以适配原生应用、语言绑定和浏览器场景：
...
面向 Rust、Python、Node.js、WebAssembly 和未来语言绑定的稳定 C-facing ABI。它使用不透明句柄、显式错误对象、参数值构建器，以及可遍历 list、map、node、edge 的嵌套 value ref。
...
ZYX 通过 Emscripten 编译为 WebAssembly。浏览器 Playground 使用 Driver ABI TypeScript 包装器和只读事务，在浏览器环境中保证安全性。
...
构建系统：**CMake** + **Conan** + **Ninja**。测试：**Google Test**，覆盖率目标 95%+。
