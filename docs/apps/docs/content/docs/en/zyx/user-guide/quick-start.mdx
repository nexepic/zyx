...
Download a pre-built executable from the [GitHub Releases](https://github.com/nexepic/zyx/releases) page.

### Build from Source (optional)

See [Installation Guide](../contributing/installation) for full instructions.
...
:::tip Shorthand
`zyx database open ./demo.zyx -c` is the short form of `--create-if-missing`. It is especially useful in scripts and automation.
:::
...
If you prefer Python, install the `zyxdb` package and interact with ZYX programmatically:
...
with zyxdb.Database("/tmp/demo.zyx") as db:
    # Create nodes (returns node IDs)
    alice = db.create_node("User", {"name": "Alice", "age": 30})
    bob = db.create_node("User", {"name": "Bob", "age": 25})

    # Create an edge
    db.create_edge(alice, bob, "KNOWS", {"since": 2026})
...
:::tip
For the full Python API reference, see [Python API Reference](../api/python-api).
:::
