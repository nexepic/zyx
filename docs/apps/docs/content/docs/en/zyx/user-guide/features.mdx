...
|-----------|-----------|-------------|
| Graph Projection Catalog | `gds.graph.*` | Named filtered graphs, lifecycle, schema, and memory metrics |
| PageRank | `gds.pageRank.stream` | Iterative importance scoring |
| Leiden Community Detection | `gds.leiden.stream` | Leiden algorithm with refinement and level lifting |
...
CALL gds.graph.project('social', {
    nodeLabels: 'Person',
    relationships: {KNOWS: {orientation: 'NATURAL'}}
})
...
```cypher
CALL gds.pageRank.stream('social', 20, 0.85)
YIELD nodeId, score
RETURN nodeId, score ORDER BY score DESC LIMIT 10
```

See [GDS Graph Projections](/en/docs/zyx/algorithms/graph-projections) for projection configuration and catalog lifecycle.
...
ZYX provides API surfaces for native applications, language bindings, and browser use:
...
Stable C-facing ABI for Rust, Python, Node.js, WebAssembly, and future language bindings. It uses opaque handles, explicit error objects, value builders for parameters, and nested value references for lists, maps, nodes, and edges.

```c
#include <zyx/zyx_driver_abi.h>

zyx_driver_db_t *db = NULL;
zyx_driver_error_t *error = NULL;
zyx_driver_db_open("./my.zyx", &db, &error);

zyx_driver_result_t *result = NULL;
zyx_driver_db_execute(db, "MATCH (n:Person) RETURN n.name", NULL, &result, &error);

while (zyx_driver_result_next(result, &error) == ZYX_DRIVER_ROW) {
    const char *name = NULL;
    zyx_driver_result_get_string(result, 0, &name, &error);
    printf("%s\n", name);
}
zyx_driver_result_free(result);
zyx_driver_db_close(db, &error);
```

### WebAssembly

ZYX compiles to WebAssembly via Emscripten. The browser playground uses the Driver ABI TypeScript wrapper and read-only transactions to enforce safety in the browser environment.
...
Build system: **CMake** + **Conan** + **Ninja**. Tests: **Google Test** with 95%+ coverage target.
