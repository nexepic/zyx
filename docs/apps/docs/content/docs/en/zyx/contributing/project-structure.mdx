...
```cpp
include/zyx/
├── zyx.hpp              # Main C++ public header
├── zyx_driver_abi.h     # Stable C-facing Driver ABI header
└── value.hpp            # Public value type definitions
```

`zyx_driver_abi.h` is the only supported C-facing ABI header.
...
├── api/                  # C++ API and Driver ABI implementation
...
CMake discovers `src/**/*.cpp` and `tests/**/test_*.cpp` automatically via `cmake/SourceSets.cmake`, so new source and test files are picked up on the next configure/build.
...
- [Testing Guidelines](/en/docs/zyx/contributing/testing) - Testing practices
