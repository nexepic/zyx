...
Metrix 使用基于链表的邻接结构实现高性能关系遍历系统。这为 Cypher 查询如 `MATCH (a)-[:KNOWS]->(b)` 提供了高效的图关系导航能力，并实现 O(1) 的连接节点和边访问。
...
Metrix 使用基于链表的邻接结构，其中每个节点维护指向其第一条出边和入边的指针：
