...
graph TB
    subgraph "码本"
        C1["码本 1<br/>(256 × 8 浮点数)"]
        C2["码本 2<br/>(256 × 8 浮点数)"]
        C3["码本 3<br/>(256 × 8 浮点数)"]
        C4["码本 M<br/>(256 × 8 浮点数)"]
    end
...
flowchart TD
    ...
    Precompute --> Table["表 T[M×K]<br/>~96 KB"]

    Database["数据库向量 x<br/>(PQ 码)"] --> Lookup[从表中查找距离]
