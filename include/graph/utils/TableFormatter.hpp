...
namespace graph::storage {
	...
	private:
		...
		void printHorizontalLine(std::ostream &os, const std::string &left, const std::string &mid,
	...
	public:
		...
		void setTitle(const std::string &title_) { this->title = title_; }
		...
		void print(std::ostream &os = std::cout) const {
			...
			os << vertical;
			for (const auto &col: columns) {
				os << std::string(padding, ' ') << std::left << std::setw(static_cast<int>(col.width))
				   << std::setfill(' ') << col.header << std::string(padding, ' ') << vertical;
			}
			os << std::endl;
			...
			for (size_t row = 0; row < columns[0].values.size(); ++row) {
				...
				for (const auto &col: columns) {
					os << std::string(padding, ' ') << std::left << std::setw(static_cast<int>(col.width))
					   << std::setfill(' ') << col.values[row] << std::string(padding, ' ') << vertical;
				}
				os << std::endl;
			}
