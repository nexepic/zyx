/**
 * @file LogicalExplain.hpp
 * @brief Logical operator for EXPLAIN — wraps an inner plan to show without executing.
 *
...
public:
	explicit LogicalExplain(std::unique_ptr<LogicalOperator> innerPlan)
		: innerPlan_(std::move(innerPlan)) {}

	[[nodiscard]] LogicalOpType getType() const override { return LogicalOpType::LOP_EXPLAIN; }
	...
	[[nodiscard]] std::unique_ptr<LogicalOperator> clone() const override {
		return std::make_unique<LogicalExplain>(innerPlan_ ? innerPlan_->clone() : nullptr);
	}
	...
	[[nodiscard]] LogicalOperator *getInnerPlan() { return innerPlan_.get(); }
