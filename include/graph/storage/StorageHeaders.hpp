...
namespace graph::storage {
	...
	struct alignas(SEGMENT_HEADER_SIZE) SegmentHeader {
		...
		[[nodiscard]] uint32_t getActiveCount() const { return calculateActiveCount(*this); }

		[[nodiscard]] uint32_t getTotalFreeSpace() const { return calculateTotalFreeSpace(*this); }

		[[nodiscard]] double getFragmentationRatio() const { return calculateFragmentationRatio(*this); }
	};
