...
namespace graph::query {
	...
	public:
		...
		void addParameter(const std::string& key, double value) {
			doubleParams_[key] = value;
		}

		void addParameter(const std::string& key, uint64_t value) {
			uint64Params_[key] = value;
		}
		...
		[[nodiscard]] const std::unordered_map<std::string, double>& getDoubleParams() const {
			return doubleParams_;
		}

		[[nodiscard]] const std::unordered_map<std::string, uint64_t>& getUint64Params() const {
			return uint64Params_;
		}
	...
	public:
		...
		QueryPlan createPlanForLabelAndPropertyQuery(const std::string& label,
												   const std::string& key,
												   const std::string& value);

		// Create a plan for finding nodes by property range
		QueryPlan createPlanForPropertyRangeQuery(const std::string& key,
												double minValue,
												double maxValue);

		// Create a plan for text search
		QueryPlan createPlanForTextSearchQuery(const std::string& key,
											 const std::string& searchText);

		// Create a plan for finding relationships
		QueryPlan createPlanForRelationshipQuery(uint64_t nodeId,
											   const std::string& edgeLabel);

		// Create a plan for finding connected nodes
		QueryPlan createPlanForConnectedNodesQuery(uint64_t nodeId,
												 const std::string& edgeLabel,
