...
namespace graph::storage {
    ...
    void SegmentIndexManager::updateSegmentIndex(const SegmentHeader &header, int64_t oldStartId) {
        ...
        if (oldStartId != -1 && oldStartId != header.start_id) {
            ...
            auto itOld = std::lower_bound(segmentIndex.begin(), segmentIndex.end(), oldKey,
                [](const SegmentIndex &a, const SegmentIndex &b) {
                    return a.startId < b.startId;
                });

            // Verify and Remove
            if (itOld != segmentIndex.end() && itOld->startId == oldStartId && itOld->segmentOffset == header.file_offset) {
                ...
                std::erase_if(segmentIndex, [&](const SegmentIndex& idx) {
                    return idx.segmentOffset == header.file_offset;
                });
            }
        ...
        auto itNew = std::lower_bound(segmentIndex.begin(), segmentIndex.end(), newIndex,
                [](const SegmentIndex &a, const SegmentIndex &b) {
                    return a.startId < b.startId;
                });
    ...
    std::vector<SegmentIndexManager::SegmentIndex> &SegmentIndexManager::getSegmentIndexForType(uint32_t type) {
        switch (type) {
            case toUnderlying(SegmentType::Node): return nodeSegmentIndex_;
            case toUnderlying(SegmentType::Edge): return edgeSegmentIndex_;
            case toUnderlying(SegmentType::Property): return propertySegmentIndex_;
            case toUnderlying(SegmentType::Blob): return blobSegmentIndex_;
            case toUnderlying(SegmentType::Index): return indexSegmentIndex_;
            case toUnderlying(SegmentType::State): return stateSegmentIndex_;
