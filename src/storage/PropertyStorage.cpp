...
PropertyReference PropertyStorage::storeProperties(
    ...
    if (dataSize > storage::MAX_SEGMENT_PROPERTY_SIZE) {
        ...
        return PropertyReference{
           PropertyReference::StorageType::BLOB,
           blobId,
    ...
    if (segmentOffset == 0) {
        ...
        if (propertySegmentHead == 0) {
            // propertySegmentHead = segmentOffset;
    ...
    segmentHeader.segment_crc = utils::calculateCrc(&segmentHeader,
                                                  offsetof(storage::PropertySegmentHeader, segment_crc));
...
std::unordered_map<std::string, PropertyValue> PropertyStorage::loadProperties(
...
uint64_t PropertyStorage::allocatePropertySegment(
	const std::shared_ptr<std::fstream> &file,
    uint32_t capacity) {
...
uint32_t PropertyStorage::calculateSerializedSize(
    const std::unordered_map<std::string, PropertyValue>& properties) {
...
std::pair<uint64_t, uint32_t> PropertyStorage::findAvailableSpace(
	const std::shared_ptr<std::fstream> &file,
