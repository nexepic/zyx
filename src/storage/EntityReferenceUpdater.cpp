...
namespace graph::storage {
    ...
    void EntityReferenceUpdater::updateEntityReferences(int64_t oldEntityId, const void *newEntity, uint32_t entityType) {
        switch (entityType) {
    ...
    void EntityReferenceUpdater::updateNodeReferences(int64_t oldNodeId, const Node &newNode) {
        ...
        while (currentEdgeId != 0) {
            ...
            if (edge.getId() == 0 || !edge.isActive()) break;
    ...
    void EntityReferenceUpdater::updateEdgeReferences(int64_t oldEdgeId, const Edge &newEdge) {
        ...
        auto updateNeighbor = [&](int64_t neighborId, bool isPrev, bool isOutChain) {
            ...
            if (neighbor.getId() == 0 || !neighbor.isActive()) return;
            ...
            if (changed) dm->updateEdge(neighbor);
        };
    ...
    void EntityReferenceUpdater::updatePropertyReferences(int64_t oldPropId, const Property &newProperty) const {
        ...
        if (entityType == toUnderlying(SegmentType::Node)) {
            ...
            if (node.getId() != 0 && node.isActive() &&
                node.getPropertyEntityId() == oldPropId &&
            ...
            if (edge.getId() != 0 && edge.isActive() &&
                edge.getPropertyEntityId() == oldPropId &&
    ...
    void EntityReferenceUpdater::updateBlobReferences(int64_t oldBlobId, const Blob &newBlob) const {
        ...
        if (entityType == toUnderlying(SegmentType::Node)) {
            Node node = dm->getNode(entityId);
            if (node.getId() != 0 && node.isActive() &&
                node.getPropertyEntityId() == oldBlobId &&
            ...
            Edge edge = dm->getEdge(entityId);
            if (edge.getId() != 0 && edge.isActive() &&
                edge.getPropertyEntityId() == oldBlobId &&
        ...
        if (nextBlobId != 0) updateBlobChainReference(oldBlobId, newBlobId, nextBlobId, true, newBlob);

        int64_t prevBlobId = newBlob.getPrevBlobId();
        if (prevBlobId != 0) updateBlobChainReference(oldBlobId, newBlobId, prevBlobId, false, newBlob);
    }
    ...
    void EntityReferenceUpdater::updateIndexReferences(int64_t oldIndexId, const Index &newIndex) {
        ...
        if (oldIndexId == newIndexId) return;
    ...
    void EntityReferenceUpdater::updateBlobChainReference(int64_t oldBlobId, int64_t newBlobId,
        ...
        if (linkedBlob.getId() == 0 || !linkedBlob.isActive()) return;
    ...
    void EntityReferenceUpdater::updateIndexSiblingPtr(int64_t siblingId, int64_t oldId, int64_t newId, bool updateNext) const {
        auto dm = dataManager_.lock();
        if (!dm) return;
        ...
        if (sibling.getId() != 0 && sibling.isActive()) {
            ...
            if (changed) dm->updateIndexEntity(sibling);
        }
    }
    ...
    void EntityReferenceUpdater::updateStateChainReference(int64_t targetStateId, int64_t oldId, int64_t newId, bool updateNext) const {
        auto dm = dataManager_.lock();
        if (!dm) return;
        ...
        if (state.getId() != 0 && state.isActive()) {
            ...
            if (changed) dm->updateStateEntity(state);
        }
    }
    ...
    void EntityReferenceUpdater::updatePropertiesPointingToEntity(int64_t oldEntityId, const T &newEntity) {
        ...
        if (info.storageType == PropertyStorageType::NONE || info.propertyEntityId == 0) return;
        ...
        if (info.storageType == PropertyStorageType::PROPERTY_ENTITY) {
            ...
            if (prop.getId() != 0 && prop.isActive() &&
                prop.getEntityId() == oldEntityId && prop.getEntityType() == entityType) {
            ...
            if (blob.getId() != 0 && blob.isActive() &&
                blob.getEntityId() == oldEntityId && blob.getEntityType() == entityType) {
