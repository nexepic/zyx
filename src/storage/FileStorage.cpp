...
namespace graph::storage {
	...
	void FileStorage::initializeComponents() {
		...
		spaceManager = std::make_shared<SpaceManager>(fileStream, segmentTracker, dbFilePath);
		spaceManager->initialize(currentHeader);
	...
	template<typename T>
	void FileStorage::saveData(std::unordered_map<uint64_t, T> &data, uint64_t &segmentHead, uint32_t itemsPerSegment) {
		if (data.empty())
			return;
	...
	uint64_t FileStorage::allocateSegment(uint8_t type, uint32_t capacity) const {
		...
		size_t alignedSize =
				((dataSize + currentHeader.page_size - 1) / currentHeader.page_size) * currentHeader.page_size;
	...
	void FileStorage::flush() { save(); }

	Node FileStorage::getNode(uint64_t id) {
		if (!isFileOpen) {
			open();
		}
	...
	Edge FileStorage::getEdge(uint64_t id) {
		if (!isFileOpen) {
			open();
		}
	...
	std::vector<Node> FileStorage::getNodes(const std::vector<uint64_t> &ids) {
		if (!isFileOpen) {
			open();
		}
	...
	std::vector<Edge> FileStorage::getEdges(const std::vector<uint64_t> &ids) {
		if (!isFileOpen) {
			open();
		}
	...
	std::vector<Node> FileStorage::getNodesInRange(uint64_t startId, uint64_t endId, size_t limit) {
		if (!isFileOpen) {
			open();
		}
	...
	std::vector<Edge> FileStorage::getEdgesInRange(uint64_t startId, uint64_t endId, size_t limit) {
		if (!isFileOpen) {
			open();
		}
	...
	std::vector<Edge> FileStorage::findEdgesByNode(uint64_t nodeId, const std::string &direction) {
		if (!isFileOpen) {
			open();
		}
	...
	std::unordered_map<uint64_t, Node> FileStorage::getAllNodes() {
		if (!isFileOpen) {
			open();
		}
		...
		std::unordered_map<uint64_t, Node> result;
	...
	std::unordered_map<uint64_t, Edge> FileStorage::getAllEdges() {
		if (!isFileOpen) {
			open();
		}
		...
		std::unordered_map<uint64_t, Edge> result;
