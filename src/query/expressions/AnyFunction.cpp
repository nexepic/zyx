...
PropertyValue AnyFunction::evaluate(
	...
	if (listValue.getType() != PropertyType::LIST) {
		throw ExpressionEvaluationException(
			"any() function requires a list as the second argument"
		);
	}
...
bool AnyFunction::evaluatePredicate(
	...
	// Empty list: any() returns false (no elements to satisfy)
	if (list.empty()) {
		return false;
	}
