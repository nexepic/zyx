/**
 * @file NoneFunction.cpp
 * @date 2025/3/9
 *
 * Implementation of NoneFunction
 */
...
PropertyValue NoneFunction::evaluate(
	...
	if (listValue.getType() != PropertyType::LIST) {
		throw ExpressionEvaluationException(
			"none() function requires a list as the second argument"
		);
	}
...
bool NoneFunction::evaluatePredicate(
	...
	// Empty list: none() returns true (vacuously true - no elements to satisfy)
	if (list.empty()) {
		return true;
	}
	...
	for (const auto& elem : list) {
		...
		if (isTrue) {
			return false; // Short-circuit - found a match
		}
	}
