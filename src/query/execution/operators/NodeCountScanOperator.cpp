...
namespace graph::query::execution::operators {
	...
	NodeCountScanOperator::NodeCountScanOperator(std::shared_ptr<storage::DataManager> dm,
	...
	void NodeCountScanOperator::open() {
	...
	std::optional<RecordBatch> NodeCountScanOperator::next() {
	...
	void NodeCountScanOperator::close() {
		candidateSet_ = NodeCandidateSet{};
		directCount_.reset();
		emitted_ = false;
	}

	std::vector<std::string> NodeCountScanOperator::getOutputVariables() const { return {outputAlias_}; }

	std::string NodeCountScanOperator::toString() const {
		return "NodeCountScan(" + config_.variable + " -> " + outputAlias_ + ")";
	}
