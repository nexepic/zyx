...
namespace graph::query::indexes {
    ...
    void IndexTreeManager::handleUnderflow(Index &node, int64_t &rootId) {
        ...
        if (rightSiblingId != 0) {
            ...
            if (rightSibling.getEntryCount() >= MIN_ENTRY_COUNT + 1) {
        ...
        if (leftSiblingId != 0) {
            ...
            if (leftSibling.getEntryCount() >= MIN_ENTRY_COUNT + 1) {
    ...
    bool IndexTreeManager::remove(int64_t rootId, const PropertyValue &key, int64_t value) {
        ...
        auto leaf = dataManager_->getIndex(leafId);
        // We need to know the entry count *before* removal to check for underflow.
        const uint32_t countBeforeRemove = leaf.getEntryCount();
        ...
        if (removed) {
            ...
            if (leaf.getId() != rootId && leaf.getEntryCount() < MIN_ENTRY_COUNT) {
