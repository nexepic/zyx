...
int main(int argc, char** argv) {
    ...
    auto addInsertTests = [&](const std::string& prefix, InsertIndexMode mode, std::string modeName) {
        ...
        tests.push_back(std::make_unique<CypherBatchInsertBench>(
            "Insert Cypher Batch (" + modeName + ")",
            std::string(conf::PATH_INSERT_CYPHER) + prefix + "_batch",
            1, conf::DEFAULT_DATA_SIZE, mode
        ));
        tests.push_back(std::make_unique<NativeBatchInsertBench>(
            "Insert Native Batch (" + modeName + ")",
            std::string(conf::PATH_INSERT_NATIVE) + prefix + "_batch",
            1, conf::DEFAULT_DATA_SIZE, mode
        ));
    };
    ...
    tests.push_back(std::make_unique<CypherQueryBench>(
        ...
        std::string(conf::PATH_QUERY_BASE) + "_prop",
        conf::ITERATIONS_READ, conf::DATA_SIZE_QUERY, QueryMode::PROPERTY_INDEX
    ));

    tests.push_back(std::make_unique<CypherQueryBench>(
        "Query (Label Scan)",
        std::string(conf::PATH_QUERY_BASE) + "_label",
        conf::ITERATIONS_READ / 5, conf::DATA_SIZE_QUERY, QueryMode::LABEL_SCAN
    ));

    tests.push_back(std::make_unique<CypherQueryBench>(
        "Query (Full Scan)",
        std::string(conf::PATH_QUERY_BASE) + "_full",
        conf::ITERATIONS_READ / 20, conf::DATA_SIZE_QUERY, QueryMode::FULL_SCAN
    ));
